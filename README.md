...
**1.4.1、时间复杂度的定义：**
> 算法的[时间复杂度](https://baike.baidu.com/item/%E6%97%B6%E9%97%B4%E5%A4%8D%E6%9D%82%E6%80%A7?fromtitle=%E6%97%B6%E9%97%B4%E5%A4%8D%E6%9D%82%E5%BA%A6&fromid=1894057)在计算机科学中，时间复杂性(Time Complexity, 又称时间复杂度)算法的时间复杂度是一个函数，它定性描述该算法的运行时间，也就是算法的时间量度。
<br/>记做：`T(n) = Ο(f(n))`
...
```
  ...
  3、规律：
  当n趋近于无穷大时，T(n)/f(n)的极限值为不等于零的常数，此时算法执行时间的增长率和f(n)的增长率相同，
  则称f(n)是T(n)的同数量级函数。记作 T(n) = O(f(n))
  称 O(f(n)) 为算法的渐进时间复杂度，简称时间复杂度。
  
  基于以上，我们引入了渐进时间复杂度作为时间性能分析的依据。
```
* 什么是`[大O符号(Big O notation, 大O记法)](https://baike.baidu.com/item/%E5%A4%A7O%E7%AC%A6%E5%8F%B7)`：
...
```
  ...
  2、在修改后的运行次数函数中，只保留最高阶项(幂 > 平方 > 乘数 > 常数)。
  3、如果最高阶项存在且不是1，则去除这个项相乘的常数。
  
  到到的最后结果就是函数阶，也称大O阶，也就是这个算法的时间复杂度。
  通常在不同的算法在同阶项的情况下，算法的优劣会考虑常数。
```
...
```
  若对于一个算法，T(n)的上界与问题规模n无关，则称其具有常数时间，记作O(1)时间，表示算法的运行时间为常量，在函数阶中称为常数阶。
  虽然被称为“常数时间”，运行时间本身并不必须与问题规模无关，但它的上界必须是与问题规模无关的确定值。
  常数时间也可以理解为一种特殊的线性时间，只不过它的增长率为0。
```
以下一个常数阶推导过程：
```
# 算法f(a)：计算 1+2+3+4
...
sum += 3;
sum += 4;
# --- 从算法f(a)看虽然执行了4次操作，也就是说 T(n) = 4，
# --- 但是在推导大O阶时，我们认为4次操作是算法f()a所有加法常数，然后用常数1取代加法常数，
...
# --- 从算法f(b)执行来看，虽然使用的for循环操作，但是已知问题规模n是一个固定的常数4，循环总共执行次数为常数4，
# --- 所以可认为时间复杂度为O(1)，记作：T(n) = O(1)

# 计算 1+2+3+4+...+(n-1)+n：高斯算法
int sum2 = n * (1 + n) / 2
