...
# 一 基本概念         
[👆-数据结构和算法](#数据结构和算法) &nbsp; &nbsp; &nbsp; [👇-线性表](#二-线性表)

### 1-1 浅谈数据结构
[👆-目录](#数据结构和算法) &nbsp; &nbsp; &nbsp; [👇-浅谈算法](#1-2-浅谈算法)  
**1.1.1、什么是数据结构：**
...
```
  很显然，根据链式存储结构的特点，其数据元素存储关系并不能反映其逻辑关系，因此需要用一个指针存放数据元素的地址，这样子通过地址就可以找到相关联数据元素的位置
```
...
### 1-2 浅谈算法
[👆-浅谈数据结构](#1-1-浅谈数据结构) &nbsp;&nbsp;&nbsp; [👇-算法效率的度量方法](#1-3-算法效率的度量方法)
...
并且每条指令标识一个或多个操作。
  ...
  给定的问题，是可以有多种算法来解决的。如果一个算法有缺陷，或不适合于某个问题，执行这个算法将不会解决这个问题。
...
> 算法的时间复杂度是指执行算法所需要的计算工作量（好的算法一般具有时间效率高的特点）。记做T(n)=Ο(f(n))。
...
![函数的渐近增长趋势](notes/1.3.4-\345\207\275\346\225\260\347\232\204\346\270\220\350\277\221\345\242\236\351\225\277\350\266\213\345\212\277.png)
...
### 1-4 时间复杂度
[👆-算法复杂度](#1-3-算法复杂度) &nbsp;&nbsp;&nbsp;&nbsp; [👇-空间复杂度](#1-3-空间复杂度)

### 1-5 空间复杂度
[👆-时间复杂度](#1-4-时间复杂度) &nbsp;&nbsp;&nbsp;  [👇-线性表](#二-线性表)   
**1.5.1、什么是空间复杂度：**
